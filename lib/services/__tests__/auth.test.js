...
jest.unmock('@egendata/react-native-jose')
jest.mock('../jwt.js', () => {
  const realReactNativeJose = jest.requireActual('react-native-jose')
  return {
    verify: realReactNativeJose.verify,
    // sign: jest
    //   .fn()
    //   .mockResolvedValue()
...
jest.mock('../operatorAdapter', () => {
  ...
  return {
    ...
    newCreateConnection: operatorAdapterActual.newCreateConnection,
...
describe('auth', () => {
  ...
  describe('permissions', () => {
    ...
    describe('#approveConnection', () => {
      ...
      it('calls operatorAdapter.createConnection with correct arguments for empty approved', async () => {
        await approveConnection(connectionRequest, new Map())
        expect(operatorAdapter.createConnection).toHaveBeenCalledWith(
          connectionRequest,
          undefined,
          expect.any(String),
        )
      })
      it('calls operatorAdapter.createConnection with correct arguments, some approved', async () => {
        const approved = new Map([
          ['18710e28-7d6c-49cf-941e-0f954bb179ae', true],
          ['1712ec0c-9ae6-472f-9e14-46088e51f505', true],
          ['55c24372-6956-4891-b5ff-a6cf69fb5c8b', true],
        ...
        const permissions = operatorAdapter.createConnection.mock.calls[0][1]
        expect(permissions.approved).toHaveLength(4)
      })
      it('calls operatorAdapter.createConnection with correct arguments, none approved', async () => {
        const approved = new Map([
